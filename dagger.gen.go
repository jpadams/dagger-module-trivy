...
func invoke(ctx context.Context, parentJSON []byte, parentName string, fnName string, inputArgs map[string][]byte) (any, error) {
	...
	case "Container":
		...
		case "TrivyScan":
			...
			return (*Container).TrivyScan(&parent, ctx)
		default:
	...
	case "Trivy":
		...
		case "Base":
			...
			return (*Trivy).Base(&parent, ctx)
		default:
			return nil, fmt.Errorf("unknown function %s", fnName)
		}
	case "":
		return dag.CurrentModule().WithObject(dag.TypeDef().WithObject("Container").WithFunction(dag.NewFunction("TrivyScan", dag.TypeDef().WithKind(Stringkind)))).WithObject(dag.TypeDef().WithObject("Trivy").WithFunction(dag.NewFunction("Base", dag.TypeDef().WithObject("Container")))), nil
	default:
